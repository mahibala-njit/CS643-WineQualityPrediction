...
def train_model(train_data_path, validation_data_path, output_model):
    ...
    spark = SparkSession.builder.appName("WineQualityTraining").config("spark.hadoop.fs.s3a.impl", "org.apache.hadoop.fs.s3a.S3AFileSystem").getOrCreate()

    bucketname = "winedatamahi"
    ...
    best_model_path = f"s3://winedatamahi/{output_model}"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 4:
        print("Usage: train_model.py <train_data_path> <validation_data_path> <output_model_path>")
        sys.exit(1)
